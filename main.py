...
def create_qr_pdf(no_of_qr, productName, variant, serialNumber):
    ...
    prefix = f"{variant}-{year_last_two_digits}{'955G' if productName == 'TamperLokss' else ''}"
    s_num = serialNumber 
...
def add_qr_code_to_pages(count, c, no_of_qr, prefix, s_num, page, productName):
    ...
    for layout in range(no_of_layout):
        count, c, s_num = draw_each_layout(
            count, c, no_of_qr, base_x, base_y, cordinates_for_layout[layout][0], 
            cordinates_for_layout[layout][1], cordinates_for_layout[layout][2], 
            cordinates_for_layout[layout][3], layout, prefix, s_num, productName
        )
...
def draw_each_layout(count, c, no_of_qr, base_x, base_y, start_x, end_x, start_y, end_y, layout, prefix, s_num, productName):
    ...
    for x in range(start_x, end_x):
        ...
        for y in range(start_y, end_y):
            ...
            if productName == "TamperLokss":
                qr_parameter = f"{prefix}{s_num}"
            else:
                qr_parameter = f"{prefix}{s_num}"
            ...
            s_num = increment_serial_number(s_num)
            count += 1
    return count, c, s_num

def increment_serial_number(serial):
    """
    Increment the numeric portion of the serial number intelligently while preserving the format.
    Handles prefix, numeric, and suffix correctly.
    """
    import re

    # Use regex to split the serial into prefix, numeric part, and suffix
    match = re.match(r"([A-Za-z0-9]*?)(\d+)([A-Za-z]*)$", serial)
    if not match:
        raise ValueError("Serial number format is invalid")

    prefix, numeric, suffix = match.groups()

    # Increment the numeric part and preserve its original length
    if numeric:
        numeric = str(int(numeric) + 1).zfill(len(numeric))

    # Reassemble the serial number
